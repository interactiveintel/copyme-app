...
|---|---|---|---|---|
| **A1** | **Wire Vercel Blob upload pipeline** for avatars + chat media. Sign URL on server → client PUT → store ciphertext blob URL on `Message.mediaUrls` / `User.avatarUrl`. Returns signed read URL on inbox fetch (15-min TTL). | new `src/lib/blob.ts`, `src/app/api/uploads/sign/route.ts`, edits to `/api/messages/send`, `/api/auth/phone/complete`, `ProfileScreen.tsx` | **1.5d** | follow-up to S-105 / S-133 / S-134 |
| **A2** | **Wire EXIF strip + MIME sniff into the upload route.** Libraries already exist (`lib/media/exif-strip.ts`, `lib/media/sniff.ts`); just need to call them in the signing endpoint or on receive. | `src/app/api/uploads/sign/route.ts` (calls sniff/exif-strip before issuing presigned URL) | **0.5d** | S-133 follow-up |
